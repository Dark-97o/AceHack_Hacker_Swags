...
const template = "./images/swag/Hacker-new-frame.png";
    ...
    if (image) {
      ...
      uploadedImage.onload = () => {
        ...
        if (aspectRatioImage > aspectRatioCanvas) {
          ...
          sx = (uploadedImage.width - sWidth) / 4;
          sy = 0;
        } else {
          sWidth = uploadedImage.width;
          sHeight = uploadedImage.width / aspectRatioCanvas;
          sx = 0;
          sy = (uploadedImage.height - sHeight) / 4;
        }

        const scale = 0.5; // 80% of canvas size
        const destW = canvas.width * scale;
        const destH = canvas.height * scale;
        const destX = (canvas.width - destW) / 3.5;
        const destY = (canvas.height - destH) / 3.5;
            ...
            <div className="text-center md:text-left mb-4 lg:mb-5">
              <div className="heading mb-8 md:mb-4">
                <h2 className="text-2xl md:text-4xl grad-text font-bold mb-4 mt-2">
                  AceHack 5.0 Digital Badge
                </h2>
              </div>
              <p className="lg:mb-3 mb-1 font-bold">
                Ready to rock AceHack 5.0? Show the world your excitement with
